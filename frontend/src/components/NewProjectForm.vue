    ...
    <div class="modal-content">
            ...
            <div v-if="currentStep === 3">
              <div class="form-group">
                ...
                <div class="language-selector">
                  ...
                  >
                    <img src="https://flagcdn.com/w40/us.png" alt="USA Flag" /> <span>English</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <template v-else>
        ...
        <div class="modal-body processing-view">
          <div v-if="creationStatus === 'creating' || creationStatus === 'polling'">
          ...
          <div v-if="creationStatus === 'completed'">
            ...
            <p class="status-text">
              Your project "{{ finalProjectData.name }}" has been processed successfully.
            </p>
            <button class="create-btn" @click="goToEditor">Go to Project Editor</button>
          </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleClose = () => {
      if (creationStatus.value === 'creating') return; // Prevent closing while creating
      emit('close');
    };
        ...
        try {
          ...
          if (status !== 'processing') {
            ...
            if (status === 'completed' || status === 'has_conflicts') {
              ...
              emit('processing-finished'); // Inform parent to refetch list
            } else {
              creationStatus.value = 'failed';
              processingError.value = version.processing_errors?.join('\n') || 'Analysis failed without specific errors.';
            }
          }
        } catch (error) {
          clearInterval(pollingInterval.value);
          creationStatus.value = 'failed';
          processingError.value = 'Could not fetch processing status from the server.';
        }
    ...
    const handleCreateProject = async () => {
      ...
      if (projectData.value.rawText) {
        formData.append('rawText', projectData.value.rawText);
      }
