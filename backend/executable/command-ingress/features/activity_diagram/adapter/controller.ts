...
export class ActivityDiagramController {
  ...
  public listRequirementsByVersion = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { versionId } = req.params as { [k: string]: string };
      if (!versionId) {
      ...
      const data = await this.service.listRequirementsByVersion(versionId);
  ...
  public listByQuery = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { umlId, versionId, projectId } = req.query as { [k: string]: string };
      const result = await this.service.list({ umlId, versionId, projectId });
  ...
  public getById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { id } = req.params;
      const diagram = await this.service.getById(id);
      if (!diagram) {
