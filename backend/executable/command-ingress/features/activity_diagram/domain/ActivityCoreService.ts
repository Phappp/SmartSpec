...
// }
 renderSvg(diagram: Pick<ActivityDiagramDTO, 'nodes' | 'edges' | 'name'>): string {
  ...
  // 3) Calculate node sizes and positions
  const positions: Record<string,{x:number,y:number,w:number,h:number}> = {};
  const layerGap = 120;
  const nodeGapX = 30;
  const nodeGapY = 30;
  ...
  layers.forEach((layerNodes, layerIdx) => {
    const nodeBoxes = layerNodes.map(id=>{
      const node = nodes.find(n=>n.id===id)!;
      let w = Math.max(80, (node.label?.length||id.length)*7 + 20);
      let h = 36;
      if(node.type==='start'||node.type==='end'){ const size = Math.max(w,h); w=size*1.2; h=size*1.2; }
      else if(node.type==='decision'){ w*=1.3; h*=1.3; }
      return {id,w,h};
    });

    const totalWidth = nodeBoxes.reduce((a,b)=>a+b.w,0) + nodeGapX*(nodeBoxes.length-1);
    if(totalWidth + 100 > canvasWidth) canvasWidth = totalWidth + 100;

    let startX = canvasWidth/2 - totalWidth/2;
    let yBase = 80 + layerIdx * layerGap;
    ...
    nodeBoxes.forEach(b=>{
      ...
      startX += b.w + nodeGapX;
      if(y + b.h/2 + 50 > canvasHeight) canvasHeight = y + b.h/2 + 50;
    });
  });
  ...
  const nodeEls = nodes.map(n=>{
    const p = positions[n.id]; if(!p) return '';
    const label = n.label||n.id;
    let shape='', fontSize=12;

    switch(n.type){
      case 'start':
        shape=`<circle cx="${p.x}" cy="${p.y}" r="${Math.max(12,label.length*7/2+8)}" fill="#22c55e" stroke="#0f5132" stroke-width="2"/>`;
        break;
      case 'end':
        shape=`<circle cx="${p.x}" cy="${p.y}" r="${Math.max(12,label.length*7/2+8)}" fill="#ef4444" stroke="#7f1d1d" stroke-width="2"/>`;
        break;
      case 'decision': {
        const w=p.w*0.7,h=p.h*1.3;
        shape=`<polygon points="${p.x - w/2},${p.y} ${p.x},${p.y - h/2} ${p.x + w/2},${p.y} ${p.x},${p.y + h/2}" fill="#fde68a" stroke="#b45309" stroke-width="2" stroke-linejoin="round"/>`;
        break;
      }
      case 'merge':
  ...
  // 5) Edge render using curved path
  const edgeEls = edges.map(e=>{
    ...
    return `<g>
      ...
      ${e.condition?`<text x="${textX}" y="${textY}" font-size="11" text-anchor="middle" fill="#374151">${e.condition}</text>`:""}
    </g>`;
  }).join("\n");
