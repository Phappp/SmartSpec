 ...
 */
   ...
   */
  async updateMemberRole(projectId: string,memberId: string,role: "owner" | "editor" | "viewer",userId: string): Promise<ServiceResponse> {
    ...
    if (!isOwner) {
      return new ServiceResponse(ResponseStatus.Failed,"Only the project owner can change member roles",null,403);
    }

    // tìm member
    const member = project.members.find((m) =>
      m.user_id.equals(memberId)
    );
    ...
    // Không cho đổi role của owner khác
    if (member.role === "owner" && member.user_id.toString() !== userId) {
      return new ServiceResponse(ResponseStatus.Failed,"Cannot change role of another owner",null,403);
    }
    ...
    await this.logService.createLog({
      ...
      level: "info",
      details: {
        message: `${actor?.name} changed ${targetUser?.name}'s role from ${oldRole} to ${role}`
      }
    });
    // // Notification
    // const notificationServiceDomain = new NotificationServiceImpl();
    ...
    //   `Your role in project ${project.name} changed from ${oldRole} to ${role}`
    // );

    // await notificationServiceDomain.createNotification(memberId,userId,"ROLE_UPDATE","Your role has been updated",`Your role in project ${project.name} changed from ${oldRole} to ${role}`,"");

    return new ServiceResponse(ResponseStatus.Success,"Role updated successfully",null,200);
  }
}
